	...
	if (!eth_devices) {
		...
		show_boot_progress (-64);
#if 1 //MSTAR
            {
                ...
                if (s)
                {
                    printf(" Get MAC from ethaddr env\n");
                    for (i = 0; i < 6; ++i)
                    {
                        sa[i] = s ? simple_strtoul (s, &e, 16) : 0;
                        if (s)
                        {
                            s = (*e) ? e + 1 : e;
                        }
            ...
            #if 1 //Write ethaddr to emac reg for kernel usage.
                MHal_EMAC_Write_SA1_MAC_Address(sa[0], sa[1], sa[2], sa[3], sa[4], sa[5]);
