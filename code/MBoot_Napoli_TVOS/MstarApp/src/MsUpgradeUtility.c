...
// read tee from mmc
...
#ifdef CONFIG_UBI
    snprintf(buffer,CMD_BUF, "nand read.e %x tee %x",ulMmcVAddr,ulTeePartSize);
#else
    snprintf(buffer,CMD_BUF, "mmc read.p %x tee %x",ulMmcVAddr,ulTeePartSize);
#endif
    UBOOT_DEBUG("cmd: %s \n",buffer);
    ...
    if(-1==_RawData_Authentication("tee",ulMmcVAddr))
    {
    ...
    memset(buffer,0,CMD_BUF);
    #ifdef CONFIG_UBI
    snprintf(buffer,CMD_BUF, "nand read.e %x tee %x",ulMmcVAddr,TEE_HEADER_LEN);
    #else
    snprintf(buffer,CMD_BUF, "mmc read.p %x tee %x",ulMmcVAddr,TEE_HEADER_LEN);
    #endif
    UBOOT_DEBUG("cmd: %s \n",buffer);
    run_command(buffer,0);
...
// decrypt tee
    ...
    UBOOT_DEBUG("pstUsbHder uiversion : %x \n",pstUsbHder->_sthd.uiversion);
    UBOOT_DEBUG("pstUsbHder ulltime : %llx \n",pstUsbHder->_sthd.ulltime);
    UBOOT_DEBUG("pstUsbHder ullchange_list : %llx \n",pstUsbHder->_sthd.ullchange_list);
    ...
    if(ret==0)
    {
        ...
        if(IsMmcImageCorrect==TRUE){
            ...
            UBOOT_DEBUG("pstMmcHder uiversion : %x \n",pstMmcHder->_sthd.uiversion);
            UBOOT_DEBUG("pstMmcHder ulltime : %llx \n",pstMmcHder->_sthd.ulltime);
            UBOOT_DEBUG("pstMmcHder ullchange_list : %llx \n",pstMmcHder->_sthd.ullchange_list);
            ...
            if(pstUsbHder->_sthd.ullchange_list!=DEBUG_CL)
            {
