...
#define ENV_DEBUG_TEE "dbgtee"
#define DEBUG_INFO_START_ID "MSTAR..."
#define DEBUG_INFO_END_ID "...mstar"
...
#if defined(CONFIG_MSTAR_KAISER) || defined(CONFIG_MSTAR_EDISON) || defined(CONFIG_MSTAR_EIFFEL) || defined(CONFIG_MSTAR_NUGGET)|| defined(CONFIG_MSTAR_EINSTEIN)|| defined(CONFIG_MSTAR_NAPOLI)
...
void set_nuttx_debug_info(U32 u32NuttxVirtAddr)
{
    ST_TEE_HEADER* pstTeeHd=NULL;
    U32 debug_info_addr=0;    
    char *pEnv=NULL;
    UBOOT_TRACE("IN\n");
    ...
    if(pEnv!=NULL)
    {
        ...
        if((pstTeeHd->debugflag[0]==1)&&(strcmp(pEnv,"1")==0))
        {
            debug_info_addr=_PA2VA(pstTeeHd->_sthd.uihw_aes_paddr)-DEBUG_INFO_SIZE;
            UBOOT_DEBUG("debug_info_addr : %x \n",debug_info_addr);
            strncpy((void*)(debug_info_addr), DEBUG_INFO_START_ID, strlen(DEBUG_INFO_START_ID));
            memset((void*)(debug_info_addr+strlen(DEBUG_INFO_START_ID)),1,1);
            strncpy((void*)(debug_info_addr+DEBUG_INFO_SIZE-strlen(DEBUG_INFO_END_ID)), DEBUG_INFO_END_ID, strlen(DEBUG_INFO_END_ID));
        }
    }
...
int do_bootNuttx (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    set_nuttx_debug_info(u32NuttxVirtAddr);
    ...
    if(-1==IsSecureMemDone(u32NConfigAddr))
    {
