...
//#include <linux/serial_reg.h> //AWU: use it in local folder
...
#define is_real_interrupt(irq)	(1==1)
...
static irqreturn_t serial8250_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	do {
        ...
        if (!(iir & UART_IIR_NO_INT)){
			...
			handled = 1;
            //break ; // Fix Me(SamuelHuang): it's a dirty fixing. Please find out the reason why
			// original code can't work.
			end = NULL;
		} else if (end == NULL)
			end = l;
...
static void __init serial8250_isa_init_ports(void)
{
	...
	for (i = 0; i < UART_NR; i++) {
		...
		up->tx_loadsz = 16 ; // use the 8 byte depth FIFO well
	}
