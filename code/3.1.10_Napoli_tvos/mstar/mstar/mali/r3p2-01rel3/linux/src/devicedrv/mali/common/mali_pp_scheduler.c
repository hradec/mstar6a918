...
#elif PROFILING_SKIP_PP_JOBS || PROFILING_SKIP_PP_AND_GP_JOBS
	...
	if (0 < job->uargs.fence)
	{
		...
		if (1 == err)
		{
			...
			if (0 != mali_skip_gp_pp_jobs)
			{
				err = 0;
				if (job->sync_point) mali_sync_signal_pt(job->sync_point, err);
				mali_pp_job_mark_sub_job_completed(job, MALI_TRUE); /* disguide the job as failed. */
				mali_pp_scheduler_return_job_to_user(job, MALI_FALSE); /* This will also delete the job object */
				return _MALI_OSK_ERR_OK; /* User is notified via a notification, so this call is ok */
			}
			else
...
#warning PP jobs will not be executed
...
#endif /* CONFIG_SYNC */
	{
		...
		if (0 != mali_skip_gp_pp_jobs)
		{
			int err = 0;
			if (job->sync_point) mali_sync_signal_pt(job->sync_point, err);
			mali_pp_job_mark_sub_job_completed(job, MALI_TRUE); /*  disguide the job as successs. */
			mali_pp_scheduler_return_job_to_user(job, MALI_FALSE); /* This will also delete the job object */
			return _MALI_OSK_ERR_OK; /* User is notified via a notification, so this call is ok */
		}
		else
