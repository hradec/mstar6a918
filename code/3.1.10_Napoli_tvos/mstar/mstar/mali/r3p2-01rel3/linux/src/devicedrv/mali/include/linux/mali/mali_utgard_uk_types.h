...
typedef enum
{
    ...
    _MALI_UK_MEMORY_USAGE_DUMPED,            /**< Memory Usage dumped  */
#endif
#ifdef MSTAR
    _MALI_UK_GET_CPU_ADJUST,                 /**< _mali_uku_get_cpu_adjust() */
...
typedef enum
{
	...
	_MALI_NOTIFCATION_MEMORY_USAGE_DUMP =           (_MALI_UK_MEMORY_SUBSYSTEM << 16) | 0x80,
#endif
...
#ifdef MSTAR_MALI_DUMP_MEMORY_USAGE
		_mali_uk_memory_usage_dumped_s  memory_usage_dumped; /**< [out] Notification data for _MALI_NOTIFCATION_MEMORY_USAGE_DUMP notification type */
#endif
	} data;
...
#ifdef MSTAR
/** @brief struct to get the value of cpu adjustment from the kernel space within a given context
 */
typedef struct
{
	void *ctx;                       /**< [in,out] user-kernel context (trashed on output) */
	u32 value;                       /**< [out] value of cpu adjust */
} _mali_uk_get_cpu_adjust;

/** @brief struct to get the value of miu1 physical base address from the kernel space within a given context
 */
typedef struct
{
	void *ctx;                       /**< [in,out] user-kernel context (trashed on output) */
