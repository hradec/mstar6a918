...
#ifdef MALI_SESSION_MEMORY_USAGE
	struct mali_session_data_list * list;
#endif
};
...
#ifdef MALI_SESSION_MEMORY_USAGE
static _mali_osk_lock_t* session_data_lock = NULL;

static _MALI_OSK_LIST_HEAD(session_data_head);

static _mali_osk_list_t* _mali_get_session_data_list(void)
{
	return &session_data_head;
}

static _mali_osk_errcode_t _mali_session_data_lock(void)
{
	return _mali_osk_lock_wait( session_data_lock, _MALI_OSK_LOCKMODE_RW );
}

static void _mali_session_data_unlock(void)
{
