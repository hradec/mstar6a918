...
static u32 mali_mmu_register_read_no_lock(mali_kernel_memory_mmu * unit, mali_mmu_register reg);
#endif
...
#if defined(MSTAR_RIU_ENABLED) && (MSTAR_RIU_ADDRESS_TYPE != 32)
static void mali_mmu_register_write_no_lock(mali_kernel_memory_mmu * unit, mali_mmu_register reg, u32 val);
#endif
	...
	if (0 == _mali_osk_list_empty(&session_data->memory_head))
	{
		...
		_MALI_OSK_LIST_FOREACHENTRY(descriptor, temp, &session_data->memory_head, mali_memory_allocation, list)
		{
			MALI_DEBUG_PRINT(4, ("Freeing block with mali address 0x%x size %d mapped in user space at 0x%x\n",
						descriptor->mali_address, descriptor->size, descriptor->mapping)  /* MStar modification */
					);
...
static _mali_osk_errcode_t mali_memory_core_resource_mmu(_mali_osk_resource_t * resource)
{
	...
	/* map the registers */
#ifdef MSTAR_RIU_ENABLED
	mmu->mapped_registers = (mstar_riu_address)mmu->base;
#else
...
#else
	int_stat = mali_mmu_register_read_no_lock(mmu, MALI_MMU_REGISTER_INT_STATUS);
#endif
	if (0 == int_stat)
	{
...
#else
	mali_mmu_register_write_no_lock(mmu, MALI_MMU_REGISTER_INT_MASK, 0);

	mali_mmu_register_read_no_lock(mmu, MALI_MMU_REGISTER_STATUS);
#endif
...
#else
		mali_mmu_register_write_no_lock(mmu, MALI_MMU_REGISTER_INT_CLEAR, MALI_MMU_INTERRUPT_READ_BUS_ERROR);
#endif
		/* reenable it */
#if !defined(MSTAR_RIU_ENABLED) || (MSTAR_RIU_ADDRESS_TYPE == 32)
...
#else
		mali_mmu_register_write_no_lock(mmu, MALI_MMU_REGISTER_INT_MASK, mali_mmu_register_read(mmu, MALI_MMU_REGISTER_INT_MASK) | MALI_MMU_INTERRUPT_READ_BUS_ERROR);
#endif
	}
...
#ifdef MSTAR_RIU_ENABLED
	val = _mstar_riu_read(unit->mapped_registers, (u32)reg * sizeof(u32));
#else
...
#ifdef MSTAR_RIU_ENABLED
	_mstar_riu_write(unit->mapped_registers, (u32)reg * sizeof(u32), val);
#else
...
static u32 mali_mmu_register_read_no_lock(mali_kernel_memory_mmu * unit, mali_mmu_register reg)
{
	...
	val = _mstar_riu_read_no_lock(unit->mapped_registers, (u32)reg * sizeof(u32));
...
static void mali_mmu_register_write_no_lock(mali_kernel_memory_mmu * unit, mali_mmu_register reg, u32 val)
{
	...
	_mstar_riu_write_no_lock(unit->mapped_registers, (u32)reg * sizeof(u32), val);
}
#endif
...
static _mali_osk_errcode_t mali_address_manager_map(mali_memory_allocation * descriptor, u32 offset, u32 *phys_addr, u32 size)
{
	...
	{
#if defined(MSTAR) && defined(__GNUC__)
		volatile mali_io_address addr = session_data->page_entries_mapped[MALI_MMU_PDE_ENTRY(mali_address)];

		/* XXX: "addr" could be NULL, which should not happen */
...
u32 _mali_ukk_report_session_memory_usage(void * ctx)
{
	memory_session * session_data;
	u32 sum = 0;

	MALI_CHECK_NON_NULL(ctx, _MALI_OSK_ERR_INVALID_ARGS);

	session_data = (memory_session *)mali_kernel_session_manager_slot_get(ctx, mali_subsystem_memory_id);
