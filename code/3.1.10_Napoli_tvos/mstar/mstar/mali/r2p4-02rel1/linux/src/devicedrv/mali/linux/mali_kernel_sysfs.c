...
#include <linux/mm.h>
#endif /* MALI_SESSION_MEMORY_USAGE */
...
#ifdef MALI_SESSION_MEMORY_USAGE
static int mali_seq_session_memory_usage_show(struct seq_file *seq_file, void *v)
{
	_mali_osk_list_t *data_head = _mali_get_session_data_list();
	struct mali_session_data_list *data, *temp_data;

#if defined(MSTAR) && !USING_MMU
	return -ENOTSUP;
#endif

	seq_printf(seq_file, "%10s %5s %s\n", "MEM", "PID", "COMM");

	if (_MALI_OSK_ERR_OK != _mali_session_data_lock())
	{
		return -EAGAIN;
	}

	_MALI_OSK_LIST_FOREACHENTRY(data, temp_data, data_head, struct mali_session_data_list, list_head)
	{
		struct task_struct* task = pid_task(find_vpid(data->pid), PIDTYPE_PID);

		seq_printf(seq_file, "%10u %5u %s\n", 
				  _mali_ukk_report_session_memory_usage(data->session_data), 
				  data->pid, 
				  task ? task->comm : "");
	}

	_mali_session_data_unlock();

	return 0;
}

static int mali_seq_session_memory_usage_open(struct inode *inode, struct file *file)
{
	return single_open(file, mali_seq_session_memory_usage_show, NULL);
}

static struct file_operations mali_seq_session_memory_usage_fops = {
	.owner   = THIS_MODULE,
	.open    = mali_seq_session_memory_usage_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
};
...
static ssize_t memory_used_read(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	size_t r;
#if defined(MSTAR) && !USING_MMU
	u32 mem = 0;    /* _mali_ukk_report_memory_usage is not defined if MMU is disabled. */
#else
...
#ifdef MALI_SESSION_MEMORY_USAGE
			debugfs_create_file("session_memory_usage", 0400, mali_debugfs_dir, NULL, &mali_seq_session_memory_usage_fops);
#endif
		}
	}
