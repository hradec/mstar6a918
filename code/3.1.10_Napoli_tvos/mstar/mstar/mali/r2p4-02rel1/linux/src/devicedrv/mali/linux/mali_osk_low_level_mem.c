...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)    /* MStar modification */
static DEFINE_SPINLOCK(allocation_list_spinlock);
#else
static spinlock_t allocation_list_spinlock = SPIN_LOCK_UNLOCKED;
#endif
...
#elif defined(MIPS) && defined(CONFIG_ALLOC_MALI_IN_MIU1)
	new_page = alloc_page(GFP_HIGHUSER | __GFP_ZERO | __GFP_REPEAT | __GFP_NOWARN | __GFP_COLD | __GFP_MALI_HIGHMEM);
...
#ifdef MSTAR
#if defined(CONFIG_ZONE_NORMAL_MIU1)    /* prefer to allocate 3D stuff in MIU1 */
	virt = dma_alloc_coherent(NULL, size, phys, __GFP_NORMAL_MIU1 | __GFP_NOWARN );

	if ( NULL == virt )
	{
		virt = dma_alloc_coherent(NULL, size, phys, GFP_KERNEL | GFP_DMA | __GFP_NOWARN );
	}
#elif defined(CONFIG_ALLOC_MALI_IN_MIU1)
	virt = dma_alloc_coherent(NULL, size, phys, GFP_HIGHUSER | __GFP_MALI_HIGHMEM | __GFP_NOWARN );
...
u32 inline _mstar_riu_read(volatile mstar_riu_address base, u32 offset)
{
	...
	disable_irq(MALI_IRQ);
	spin_lock(&_mstar_riu_lock);
	value = _mstar_riu_read_no_lock(base, offset);
	spin_unlock(&_mstar_riu_lock);
	enable_irq(MALI_IRQ);
