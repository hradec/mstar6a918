...
typedef struct platform_pmu
{
	...
	mstar_riu_address reg_mapped;   /**< IO-mapped pointer to registers */
#else
...
#if PMU_TEST
static void pmu_dump_regs( platform_pmu_t *pmu );   /* MStar modifications */
...
#ifdef MSTAR_RIU_ENABLED
		pmu_info->reg_mapped = (mstar_riu_address)pmu_info->reg_base_addr;
#else
...
#ifdef MSTAR_RIU_ENABLED
	read_val = _mstar_riu_read(pmu->reg_mapped, relative_address);
#else
...
#ifdef MSTAR_RIU_ENABLED
	_mstar_riu_write(pmu->reg_mapped, relative_address, new_val);
#else
...
u32 _mali_pmm_pmu_irq_power_up_clear( platform_pmu_t *pmu )
{
	MALI_DEBUG_ASSERT_POINTER( pmu );

	/* disable interrupt and clear interrupt status */
	pmu_reg_write( pmu, (u32)PMU_REG_ADDR_MGMT_INT_MASK, 0 );
	pmu_reg_write( pmu, (u32)PMU_REG_ADDR_MGMT_INT_CLEAR, 0 );
