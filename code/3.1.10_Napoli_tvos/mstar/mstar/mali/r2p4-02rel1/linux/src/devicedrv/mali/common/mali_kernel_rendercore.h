...
typedef struct mali_core_renderunit
{
	...
	mstar_riu_address registers_mapped;     /* IO-mapped pointer to registers */
#else
...
#ifdef MSTAR_RIU_ENABLED
	read_val = _mstar_riu_read(core->registers_mapped, relative_address);
#else
...
MALI_STATIC_INLINE u32 mali_core_renderunit_register_read_no_lock(struct mali_core_renderunit *core, u32 relative_address)
{
	u32 read_val;
	...
	read_val = _mstar_riu_read_no_lock(core->registers_mapped, relative_address);
...
#ifdef MSTAR_RIU_ENABLED
	_mstar_riu_write(core->registers_mapped, relative_address, new_val);
#else
...
#if defined(MSTAR_RIU_ENABLED) && (MSTAR_RIU_ADDRESS_TYPE != 32)
MALI_STATIC_INLINE void mali_core_renderunit_register_write_no_lock(struct mali_core_renderunit *core,
                                                                    u32 relative_address,
                                                                    u32 new_val)
{
#ifndef MSOS
	...
	_mstar_riu_write_no_lock(core->registers_mapped, relative_address, new_val);
}
#endif
