...
#else
	int_stat = mali_hw_core_register_read_no_lock(&mmu->hw_core, MALI_MMU_REGISTER_INT_STATUS);
#endif
	if (0 != int_stat)
	{
#if !defined(MSTAR_RIU_ENABLED) || (MSTAR_RIU_ADDRESS_TYPE == 32)
		...
		mali_hw_core_register_read(&mmu->hw_core, MALI_MMU_REGISTER_STATUS);
#else
		mali_hw_core_register_write_no_lock(&mmu->hw_core, MALI_MMU_REGISTER_INT_MASK, 0);
		mali_hw_core_register_read_no_lock(&mmu->hw_core, MALI_MMU_REGISTER_STATUS);
#endif
...
#else
			mali_hw_core_register_write_no_lock(&mmu->hw_core, MALI_MMU_REGISTER_INT_CLEAR, MALI_MMU_INTERRUPT_READ_BUS_ERROR);
#endif
			/* reenable it */
#if !defined(MSTAR_RIU_ENABLED) || (MSTAR_RIU_ADDRESS_TYPE == 32)
...
#else
			mali_hw_core_register_write_no_lock(&mmu->hw_core, MALI_MMU_REGISTER_INT_MASK,
			                                    mali_hw_core_register_read_no_lock(&mmu->hw_core, MALI_MMU_REGISTER_INT_MASK) | MALI_MMU_INTERRUPT_READ_BUS_ERROR);
#endif
