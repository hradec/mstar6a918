...
{
#if defined(MSTAR_G2) || defined(MSTAR_GLADIUS)
    void mstar_fb_getmem(unsigned long *, unsigned long *);
    int i;
    unsigned long base, size;

    mstar_fb_getmem(&base, &size);
    ...
    for (i = 0; i < num_resources; i++)
    {
        ...
        {
#if 0
            arch_config[i].base = base;
            arch_config[i].size = size;
#else
            arch_config[i].base = 0;
