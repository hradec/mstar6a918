...
static _mali_osk_errcode_t mali_l2_cache_core_create(_mali_osk_resource_t * resource)
{
	...
	/* map the registers */
#ifdef MSTAR_RIU_ENABLED
	cache->mapped_registers = (mstar_riu_address)cache->base;
#else
...
{
#ifdef MSTAR_RIU_ENABLED
	return _mstar_riu_read(unit->mapped_registers, (u32)reg * sizeof(u32));
#else
...
static _mali_osk_errcode_t mali_kernel_l2_cache_send_command(mali_kernel_l2_cache_core *cache, u32 reg, u32 val)
{
	...
	{
#ifdef MSTAR_RIU_ENABLED
		if (!(_mstar_riu_read(cache->mapped_registers , (u32)MALI400_L2_CACHE_REGISTER_STATUS * sizeof(u32)) & (u32)MALI400_L2_CACHE_STATUS_COMMAND_BUSY))
#else
