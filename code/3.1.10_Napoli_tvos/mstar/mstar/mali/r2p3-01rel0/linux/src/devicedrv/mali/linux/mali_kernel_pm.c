...
/* Early suspend hooks */
#ifdef MSTAR
static struct early_suspend mali_dev_early_suspend_blank_screen = {
	.suspend = mali_pm_early_suspend,
	.resume = mali_pm_late_resume,
	.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
};

static struct early_suspend mali_dev_early_suspend_stop_drawing = {
	.suspend = mali_pm_early_suspend,
	.resume = mali_pm_late_resume,
	.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING,
};

static struct early_suspend mali_dev_early_suspend_disable_fb = {
	.suspend = mali_pm_early_suspend,
	.resume = mali_pm_late_resume,
...
static void mali_pm_early_suspend(struct early_suspend *mali_dev)
{
	MALI_DEBUG_PRINT(2, ("PMMDEBUG: Early suspend, level = %d\n", mali_dev->level));
	...
	switch(mali_dev->level)
	{
		...
		case EARLY_SUSPEND_LEVEL_BLANK_SCREEN:
			...
			mstar_pm_set_gpu_clock(MSTAR_PM_GPU_CLK_8M);
		break;
...
static void mali_pm_late_resume(struct early_suspend *mali_dev)
{
	MALI_DEBUG_PRINT(2, ("PMMDEBUG: Late resume, level = %d\n", mali_dev->level));

#ifdef MSTAR
	switch (mali_dev->level)
	{
		case EARLY_SUSPEND_LEVEL_BLANK_SCREEN:
			MALI_DEBUG_PRINT(4, ("PMMDEBUG: Enable screen\n" ));
			break;

		case EARLY_SUSPEND_LEVEL_STOP_DRAWING:
			MALI_DEBUG_PRINT(4, ("PMMDEBUG: Enable drawing\n" ));
			break;

		case EARLY_SUSPEND_LEVEL_DISABLE_FB:
			MALI_DEBUG_PRINT(4, ("PMMDEBUG: Enable framebuffer\n" ));
			_mali_osk_lock_wait(lock, _MALI_OSK_LOCKMODE_RW);
			if (mali_device_state == _MALI_DEVICE_RESUME)
			{
				_mali_osk_lock_signal(lock, _MALI_OSK_LOCKMODE_RW);
		...
		default:
			MALI_DEBUG_PRINT(2, ("PMMDEBUG: Invalid Suspend Mode\n" ));
			break;
	}
#else
	_mali_osk_lock_wait(lock, _MALI_OSK_LOCKMODE_RW);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
#ifdef MSTAR
			register_early_suspend(&mali_dev_early_suspend_blank_screen);
			register_early_suspend(&mali_dev_early_suspend_stop_drawing);
			register_early_suspend(&mali_dev_early_suspend_disable_fb);
#else
...
#ifdef CONFIG_HAS_EARLYSUSPEND
#ifdef MSTAR
	unregister_early_suspend(&mali_dev_early_suspend_disable_fb);
	unregister_early_suspend(&mali_dev_early_suspend_stop_drawing);
	unregister_early_suspend(&mali_dev_early_suspend_blank_screen);
#else
