...
/**
 * Check that the MMU is in a good state
 *
 * @param mmu The MMU core to check
 * @return MALI_TRUE if MMU state is good, MALI_FALSE if reset is needed
 */
MALI_STATIC_INLINE mali_bool mali_mmu_check_status(struct mali_mmu_core *mmu)
{
	u32 status = mali_mmu_get_status(mmu);
