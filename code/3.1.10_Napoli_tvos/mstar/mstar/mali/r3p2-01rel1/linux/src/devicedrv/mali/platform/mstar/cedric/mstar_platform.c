 ...
 * MStar platform specific driver functions for Cedric
 */
...
#define REG_PLLCLK_BASE     0x10c00
#define PLLCLK_REG(addr)    RIU[((addr) << 1) + REG_PLLCLK_BASE] 
...
{
    mali_gpu_clock = ((GPU_CLOCK >= 208 && GPU_CLOCK <= 442) ? (GPU_CLOCK/11*11): 209);
...
void mstar_platform_power_on(void)
{
    ...
    PLLCLK_REG(0x53) = (GPU_CLOCK >= 208 && GPU_CLOCK <= 442)? ((GPU_CLOCK/11) & 0xff): 0x13;
