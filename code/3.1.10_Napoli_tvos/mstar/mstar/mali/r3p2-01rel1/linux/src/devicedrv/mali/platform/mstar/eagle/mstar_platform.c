 ...
 * MStar platform specific driver functions for Eagle
 */
...
/* helper functions */
inline static int _mstar_platform_get_gpu_clock(void)
{
    int val = G3D_REG(0x46);

    return (val & 0xff) * ((0x0400 & val) ? 3 : 6);
}

inline static void _mstar_platform_set_gpu_clock(int val)
{
    /*
...
{
    /* GPU clock is specified in mboot */
    mali_gpu_clock = _mstar_platform_get_gpu_clock();
...
{
    /* must set before power on */
    _mstar_platform_set_gpu_clock(mali_gpu_clock);
