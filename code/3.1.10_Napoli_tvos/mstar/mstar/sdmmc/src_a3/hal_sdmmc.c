...
#define WT_EVENT_READ			    4000	//(ms)
#define WT_EVENT_WRITE			    4000	//(ms)
 ...
 * @return BOOL_T  : Write Protect or not
 ----------------------------------------------------------------------------------------------------------*/
#define GPIO4_REG       GET_CARD_REG_ADDR(A_GPIO_BANK, 0x02)
...
BOOL_T Hal_SDMMC_WriteProtect(IPEmType eIP)
{
	...
	//printk("GPIO4:%x\n", CARD_REG(GPIO4_REG));
	if ( CARD_REG(GPIO4_REG) & BIT02 )
		return (TRUE);  //No Write Protect
	return (FALSE);
