...
#define TIMEOUT_CHK_BUSY 500000
    ...
    if(bIPState > 0)
    {
		...
		{
            MBXHAL_PRINT("[Fire Mail Warnning] : co-processor is fired, but is not busy, maybe fire bit is written, and re-try fire again!\n");
    	    _FIRE_C(_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID]);
		}
    }
