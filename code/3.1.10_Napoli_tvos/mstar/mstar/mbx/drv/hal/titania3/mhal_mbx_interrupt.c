...
MBX_Result _MHAL_MBXINT_SetHostCPU(MBX_CPU_ID eHostCPU)
{
    ...
    switch(eHostCPU)
    {
        ...
        case E_MBX_CPU_AEON:
            ...
            break;
        case E_MBX_CPU_MIPS:
            s32Err = request_irq(E_FIQEXPL_8051_TO_MIPS_VPE0, _MHAL_MBXINT_INTHandler, SA_INTERRUPT, "MBX_FIQ_PM2MIPS", NULL);
            if(s32Err<0)
            {
                MBXINT_PRINT("request FIQ: %x Failed!\n", MBX_INT_PM2MIPS);
                return E_MBX_UNKNOW_ERROR;
            ...
            s32Err = request_irq(E_FIQEXPL_AEON_TO_MIPS_VPE0, _MHAL_MBXINT_INTHandler, SA_INTERRUPT, "MBX_FIQ_AEON2MIPS", NULL);
            if(s32Err<0)
            {
                free_irq(E_FIQEXPL_8051_TO_MIPS_VPE0, NULL);
                MBXINT_PRINT("request FIQ: %x Failed!\n", MBX_INT_AEON2MIPS);
                return E_MBX_UNKNOW_ERROR;
            }
