...
void _MHAL_MBXINT_SECURER22MIPSHandler(unsigned long unused)
{
    if(NULL == _pMBXMsgRecvCbFunc)
    {
        return;
    ...
    _pMBXMsgRecvCbFunc(MBX_INT_SECURER22MIPS);
}

DECLARE_TASKLET(_mbxSecureR22MipsTaskletMBXINT, _MHAL_MBXINT_SECURER22MIPSHandler, 0);
...
irqreturn_t _MHAL_MBXINT_INTHandler(int irq, void *dev_id)
{
    switch(irq)
    {
        ...
        case E_FIQ_INT_AEON_TO_MIPS:
            ...
            break;
		case E_FIQ_INT_SECURER2_TO_MIPS: //
			tasklet_schedule(&_mbxSecureR22MipsTaskletMBXINT);
            MHAL_MBXINT_Clear(MBX_INT_SECURER22MIPS);
			break;
    }
...
MBX_Result _MHAL_MBXINT_SetHostCPU(MBX_CPU_ID eHostCPU)
{
    ...
    switch(eHostCPU)
    {
        ...
        case E_MBX_CPU_MIPS:
            ...
            s32Err = request_irq(E_FIQ_INT_SECURER2_TO_MIPS, _MHAL_MBXINT_INTHandler, SA_INTERRUPT, "MBX_FIQ_AEON2MIPS", NULL);
            if(s32Err<0)
            {
                free_irq(E_FIQ_INT_8051_TO_MIPS, NULL);
                MBXINT_PRINT("request FIQ: %x Failed!\n", MBX_INT_AEON2MIPS);
			...
			MHAL_MBXINT_Enable(MBX_INT_SECURER22MIPS, TRUE);
            _MHAL_MBXINT_SetIntFlg(MBX_INT_SECURER22MIPS);
            break;
        default:
...
MBX_Result MHAL_MBXINT_Fire (MBX_CPU_ID dstCPUID, MBX_CPU_ID srcCPUID)
{
    ...
    switch(srcCPUID)
    {
        ...
        case E_MBX_CPU_MIPS:
			...
			{
				CPU_INT_REG(REG_INT_H3FIRE) = INT_H3_H2;
			}
            mb();
