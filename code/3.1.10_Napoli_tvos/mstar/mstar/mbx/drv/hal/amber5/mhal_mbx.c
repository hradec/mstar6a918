...
//=============================================================================
static MS_BOOL _MHAL_MBX_ChkFrc(MBX_ROLE_ID eSrcRole, MBX_ROLE_ID eDstRole);
...
//=============================================================================
//-------------------------------------------------------------------------------------------------
/// Check Mailbox Commication with FRC.
/// @param  eSrcRole                  \b IN: Source Role ID 
/// @param  eDstRole                  \b IN: Destination Role ID
/// @return TRUE : Communicate with FRC
/// @return FALSE : Do not communicate with FRC 
/// @attention
/// <b>[MXLIB] <em> </em></b>
//-------------------------------------------------------------------------------------------------
MS_BOOL _MHAL_MBX_ChkFrc(MBX_ROLE_ID eSrcRole, MBX_ROLE_ID eDstRole)
{
    if((eSrcRole==E_MBX_ROLE_HK&&eDstRole==E_MBX_ROLE_CP)||(eSrcRole==E_MBX_ROLE_CP&&eDstRole==E_MBX_ROLE_HK))
        return TRUE;
    return FALSE;
}
...
void _MHAL_MBX_FireMsg (MBX_Msg* pMbxMsg, MBX_ROLE_ID eSrcRole)
{
    ...
    if(_MHAL_MBX_ChkFrc(eSrcRole,pMbxMsg->eRoleID))
    {
        if(pMbxMsg->u8ParameterCount>8)
            pMbxMsg->u8ParameterCount = 8;
    }
...
{
    MS_S32 s32Idx,s32Max;

    mb();
    s32Max = (_MHAL_MBX_ChkFrc(pMbxMsg->eRoleID,eDstRole)==TRUE)? 8 : 10;
    ...
    for(s32Idx=0;  s32Idx<s32Max; s32Idx++)
    {
        pMbxMsg->u8Parameters[s32Idx] = REG8_MBX_GROUP(_u16MbxGroupIdMBXHAL[pMbxMsg->eRoleID][eDstRole], s32Idx+REG8_MBX_PARAMETER_S);
        mb();
    }
    
    if(_MHAL_MBX_ChkFrc(pMbxMsg->eRoleID,eDstRole))
    {
        pMbxMsg->u8S0 = REG8_MBX_GROUP(_u16MbxGroupIdMBXHAL[pMbxMsg->eRoleID][eDstRole], REG8_MBX_FRC_STATE_0);
        pMbxMsg->u8S1 = REG8_MBX_GROUP(_u16MbxGroupIdMBXHAL[pMbxMsg->eRoleID][eDstRole], REG8_MBX_FRC_STATE_1);
    }
    else
    {
...
#define TIMEOUT_CHK_BUSY 500000
    ...
    if(bIPState > 0)
    {
        if(_MHAL_MBX_ChkFrc(eSrcRole,pMbxMsg->eRoleID))
        {
            bIPState = _F_BUSY(_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID]);
        }
        else
        {
    ...
    if(_MHAL_MBX_ChkFrc(eSrcRole,pMbxMsg->eRoleID))
    {
        _F_S1_C(_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID]);
    }
    else
    {
	...
	if(_MHAL_MBX_ChkFrc(eSrcRole,pMbxMsg->eRoleID))
    {
    	_F_BUSY_S(_u16MbxGroupIdMBXHAL[eSrcRole][pMbxMsg->eRoleID]);
    }
    else
    {
...
MBX_Result MHAL_MBX_GetFireStatus(MBX_ROLE_ID eSrcRole, MBX_ROLE_ID eDstRole, MBXHAL_Fire_Status *pFireStatus)
{
    ...
    else
    {
        ...
        if(_MHAL_MBX_ChkFrc(eSrcRole,eDstRole))
        {
            if(_F_OVERFLOW(_u16MbxGroupIdMBXHAL[eSrcRole][eDstRole]))
            {//If Overflow:
                *pFireStatus = E_MBXHAL_FIRE_OVERFLOW;
            }
            else if(_F_DISABLED(_u16MbxGroupIdMBXHAL[eSrcRole][eDstRole]))
            {//If Disabled:
        ...
        {
            if(_OVERFLOW(_u16MbxGroupIdMBXHAL[eSrcRole][eDstRole]))
            {//If Overflow:
                *pFireStatus = E_MBXHAL_FIRE_OVERFLOW;
            }
...
MBX_Result MHAL_MBX_Recv(MBX_Msg* pMbxMsg, MBX_ROLE_ID eDstRole)
{
    ...
    if(_MHAL_MBX_ChkFrc(pMbxMsg->eRoleID,eDstRole))
    {
        _F_BUSY_S(_u16MbxGroupIdMBXHAL[pMbxMsg->eRoleID][eDstRole]);
    }
    else
    {
    ...
    if(_MHAL_MBX_ChkFrc(eSrcRole,eDstRole))
    {
        switch(eRecvSatus)
        {
            case E_MBXHAL_RECV_OVERFLOW:
                //Set OverFlow & Error:
                _F_OVERFLOW_S(_u16MbxGroupIdMBXHAL[eSrcRole][eDstRole]);
                _F_ERR_S(_u16MbxGroupIdMBXHAL[eSrcRole][eDstRole]);
                mb();
                break;
            case E_MBXHAL_RECV_DISABLED:
                //Set Disable & Error:
                _F_DISABLED_S(_u16MbxGroupIdMBXHAL[eSrcRole][eDstRole]);
                _F_ERR_S(_u16MbxGroupIdMBXHAL[eSrcRole][eDstRole]);
                mb();
                break;
            default:
        ...
        /* clear busy bit */
        _F_BUSY_C(_u16MbxGroupIdMBXHAL[eSrcRole][eDstRole]);
    }
    else
    {
        switch(eRecvSatus)
