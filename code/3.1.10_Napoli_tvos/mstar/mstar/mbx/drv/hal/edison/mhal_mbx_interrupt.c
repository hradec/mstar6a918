...
MBX_Result _MHAL_MBXINT_SetHostCPU(MBX_CPU_ID eHostCPU)
{
	...
	s32Err = request_irq(E_FIQEXPL_SECURER2_TO_ARM, _MHAL_MBXINT_INTHandler, SA_INTERRUPT, "MBX_FIQ_R2toMIPS", NULL);
            if(s32Err<0)
            {
            ...
            _MHAL_MBXINT_SetIntFlg(E_FIQEXPL_SECURER2_TO_ARM);
