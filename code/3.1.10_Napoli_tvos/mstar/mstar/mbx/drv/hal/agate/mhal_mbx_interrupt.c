...
MBX_Result MHAL_MBXINT_Fire (MBX_CPU_ID dstCPUID, MBX_CPU_ID srcCPUID)
{
    ...
    switch(srcCPUID)
    {
        case E_MBX_CPU_PM:
            ...
            {
                CPU_INT_REG(REG_INT_PMFIRE) = INT_PM_MIPS;
            }
            mb();
        ...
        case E_MBX_CPU_MIPS:
            ...
            {
                CPU_INT_REG(REG_INT_MIPSFIRE) = INT_MIPS_PM;
                mb();
                CPU_INT_REG(REG_INT_MIPSFIRE) = 0;
                mb();
            }
            else
            {
                FRC_CPU_INT_REG(REG_FRC_INT_MIPSFIRE) = INTFRC_MIPS_FRCR2;//@@@
                mb();
                FRC_CPU_INT_REG(REG_FRC_INT_MIPSFIRE) = 0;
                mb();
            }
            break;
        default:
