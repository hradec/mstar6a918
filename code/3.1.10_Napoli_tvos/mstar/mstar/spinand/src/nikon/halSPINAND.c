...
static U32 _HAL_SPINAND_BDMA_READ(U16 u16Addr, U32 u32DataSize, U8 *u8pData)
{
    ...
    {
        BDMA_WRITE(0x12, 0x4145);
        u32Addr1 -= MIPS_MIU1_BASE;
    }
    else
    {
        BDMA_WRITE(0x12, 0x4045);
...
U32 HAL_SPINAND_Write(U32 u32_PageIdx, U8 *u8Data, U8 *pu8_SpareBuf)
{
    ...
    if (_HAL_FSP_ChkWaitDone() == FALSE)
    {
        DEBUG_SPINAND(E_SPINAND_DBGLV_ERR, printk("W Wait FSP Done Time Out !!!!\r\n"));
        return ERR_SPINAND_TIMEOUT;
    }
...
U32 HAL_SPINAND_Read (U32 u32Addr, U32 u32DataSize, U8 *pu8Data)
{
    ...
    else
    {
        ...
        {
            U16 u16DataWidth, ISPDATA;
            u16Addr = CLK_READ(0x27);
            u16DataWidth = BDMA_READ(0x02);
            ISPDATA = READ_WORD((_hal_fsp.u32FspBaseAddr - 0x200));
            printk("R Wait BDMA Done Time Out CLK %x Width %x ISPDATA %x !!!!\r\n", u16Addr, u16DataWidth, ISPDATA);
        }
    }
    return ret;
}
