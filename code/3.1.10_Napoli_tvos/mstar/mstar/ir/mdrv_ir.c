...
int key_dispatch(void *p)
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            ret = wait_for_completion_interruptible_timeout(&key_completion, msecs_to_jiffies(220));
        }
        if (ret < 0)
        {
...
{
	printk(KERN_ALERT "###################################################\n");
	printk(KERN_ALERT "func: %s, line: %d, IR_MODE_SEL = %d \n", __func__, __LINE__, IR_MODE_SEL);
	printk(KERN_ALERT "###################################################\n");
    int result;
