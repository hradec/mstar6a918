...
void MHal_EMAC_Power_On_Clk( void )
{
...
#ifdef CONFIG_ETHERNET_ALBANY
    //Enable analog macro
    uRegVal = MHal_EMAC_ReadReg8(0x1219, 0x04);
    uRegVal &= 0xDF;
    MHal_EMAC_WritReg8(0x1219, 0x4, uRegVal);

    //PD_ETH33
    uRegVal = MHal_EMAC_ReadReg8(0x101E, 0x38);
    uRegVal &= 0xFE;
    MHal_EMAC_WritReg8(0x101E, 0x38, uRegVal);
    ...
    //disable EEE
    uRegVal = MHal_EMAC_ReadReg8(0x1218, 0x2D);
    uRegVal |= 0x40;
    MHal_EMAC_WritReg8(0x1218, 0x2D, uRegVal);
...
void MHal_EMAC_Power_Off_Clk( void )
{
    ...
    //Power-down LPF_OP
    MHal_EMAC_WritReg8(0x1219, 0xd4, 0x20);

    //Down PD_TX_IDAC, PD_TX_LD = 0
    MHal_EMAC_WritReg8(0x121a, 0x3a, 0xf3);
    MHal_EMAC_WritReg8(0x121a, 0x3b, 0x03);

    //Down VBUF
    MHal_EMAC_WritReg8(0x121a, 0xa1, 0x10);

    //Power-down ADCPL
    MHal_EMAC_WritReg8(0x1219, 0xcd, 0xd9);

    //Down PORST
    MHal_EMAC_WritReg8(0x1219, 0xea, 0x60);

    //Power-on REF keep on
    MHal_EMAC_WritReg8(0x1219, 0xbb, 0x05);

    //Power-on BGAP keep on
    MHal_EMAC_WritReg8(0x1219, 0xcb, 0x11);

    //Power-down LDO
    MHal_EMAC_WritReg8(0x1219, 0xfc, 0x02);
    MHal_EMAC_WritReg8(0x1219, 0xfd, 0x01);

    //100gat =1
    MHal_EMAC_WritReg8(0x121a, 0xc5, 0x40);

    //200gat =1
    MHal_EMAC_WritReg8(0x121a, 0x30, 0x53);
#endif
}
