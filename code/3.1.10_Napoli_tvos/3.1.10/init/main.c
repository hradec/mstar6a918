...
#if (MP_PLATFORM_VERIFY_LX_MEM_ALIGN == 1)
void mstar_lx_mem_alignment_check(void);
#endif //(MP_PLATFORM_VERIFY_LX_MEM_ALIGN == 1)
...
#if defined(CONFIG_MP_PLATFORM_MIPS_SYSTEM_CALL_GET_VERSION)
char gMbooVersion[50];
#endif /* CONFIG_MP_PLATFORM_MIPS_SYSTEM_CALL_GET_VERSION */
 ...
 */
	...
	parse_args("Booting kernel", static_command_line, __start___param,
        ...
        extern char *prom_getenv(char *name);
            char *ver1=NULL;
            int len=0;
            memset( (void*)gMbooVersion   ,  '\0'   , 50   );
            ver1 = prom_getenv("MBoot_version");
            //printk("\n\n\njasonfirst get mboot version:%s  ,%d\n" , ver1 , strlen(ver1)  );
            if( ver1 != NULL   )
            {
                len = strlen(ver1) + 1;
                if( len > 0 )
                {
                        memcpy( (void*) gMbooVersion   , (void*)ver1   , len  );
                        //printk("first get mboot version:%s\n" , gMbooVersion   );
                }
            }
        }
	#endif /* CONFIG_MP_PLATFORM_MIPS_SYSTEM_CALL_GET_VERSION */
...
# endif
    ...
    printk("[AT][KR][start init][%u]\n", PiuTime);
    printk("[AutoTest][Kernel][start Initprocess][%u]\n", PiuTime);
#endif
...
{
#ifdef CONFIG_MP_WDT_OFF_DBG
# if defined(CONFIG_MSTAR_KERNEL_PANIC_DO_WDT_RESET) || \
	defined(CONFIG_MSTAR_KERNEL_FAULT_DO_WDT_RESET)
	/* disable mstar wdt if debug mode */
	if (kernel_level(__LINE__, __FUNCTION__)) 
		arch_disable_wdt();
# endif
#endif
	/*
