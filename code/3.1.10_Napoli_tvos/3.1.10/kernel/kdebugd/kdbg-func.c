...
struct sec_kdbg_pc_info {
	...
	ktime_t pc_time;	/* time of the program counter */
	unsigned int cpu;
};
#endif /* CONFIG_ELF_MODULE */
...
static void __show_user_maps_with_pid(void)
{
	...
	PRINT_KD("       Total VMA Size  : %lu kB\n",
			P2K(tsk->mm->total_vm - tsk->mm->reserved_vm));
	PRINT_KD("==============================================\n");
...
static enum hrtimer_restart show_pc(struct hrtimer *timer)
{
	...
	if (prev_pc != cur_pc) {
...
#if 0
			sec_pc_trace_info[sec_pc_trace_idx].pc_time.tv.sec =
				now.tv.sec;
			sec_pc_trace_info[sec_pc_trace_idx].pc_time.tv.nsec =
				now.tv.nsec;
#else
			sec_pc_trace_info[sec_pc_trace_idx].pc_time.tv64 =
				now.tv64;
#endif
		...
		/* 		PRINT_KD ("\n"); */
		PRINT_KD
			("[kdebugd] [CPU %d] Pid:%d  PC:0x%08x\t\t(TIME:%d.%d)\n",
			 cpu, trace_tsk_pid, cur_pc, now.tv.sec, now.tv.nsec);
#endif /* CONFIG_ELF_MODULE */
	}
...
static int sec_pc_trace_pc_symbol(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		for (ii = 0; ii < buffer_count; ++ii, ++rd_idx) {
			...
			PRINT_KD
				("[kdebugd] [CPU %d] Pid:%d  PC:0x%08x (TIME:%06lld.%-9lld)  %s\n",
				 sec_pc_trace_info[rd_idx].cpu, trace_tsk_pid,
				 sec_pc_trace_info[rd_idx].pc,
				 sec_pc_trace_info[rd_idx].pc_time.tv64,
				 sec_pc_trace_info[rd_idx].pc_time.tv64,
/* 				 sec_pc_trace_info[rd_idx].pc_time.tv.sec, */
/* 				 sec_pc_trace_info[rd_idx].pc_time.tv.nsec, */
...
#ifdef CONFIG_KDEBUGD_HUB_DTVLOGD
	kdbg_register("HUB: DTVLOGD_EMERGENCY_LOG PRINTING", dtvlogd_buffer_printf, NULL, KDBG_MENU_DTVLOGD_EMERGENCY_LOG);
#endif
