...
static int kdbg_elf_sym_list_elf_db(void)
{
	...
	sym_printk("Symbol list using list_for_each_safe()\n");
...
int kdbg_elf_prune_symstr_for_func(kdbg_elf_usb_elf_list_item *plist)
{
	...
	for (si = 0; si < plist->elf_symbol_count; si++, psyms++) {
		...
		if (p_arr[new_index] < newSymBuffSize) {
			/* Although strcpy is not recommended, but it has been taken care for no mem leak */
			strcpy(symstrbuf + p_arr[new_index], plist->sym_buff + psyms->st_name);
		}
