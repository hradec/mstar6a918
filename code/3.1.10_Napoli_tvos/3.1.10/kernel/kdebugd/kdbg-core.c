...
#define KDBG_AUTO_KEY_TEST_NAME	"kdebugd/auto_key_test"
...
static int kdbg_auto_key_test_activate(void)
{
	...
	if (!kdbg_auto_key_test_proc_dentry) {
		kdbg_auto_key_test_proc_dentry =
		    create_proc_entry(KDBG_AUTO_KEY_TEST_NAME, 0, NULL);

		if (kdbg_auto_key_test_proc_dentry) {
			kdbg_auto_key_test_proc_dentry->read_proc =
			    kdbg_auto_key_test_read;
			kdbg_auto_key_test_proc_dentry->write_proc =
			    kdbg_auto_key_test_write;
		} else {
			printk(KERN_WARNING
