...
void sec_cpuusage_interrupt()
{
	...
	if (unlikely(g_cpuusage_interrupt_first_flag)) {
		...
		for_each_online_cpu(cpu) {
			...
			prev_cpu_data[cpu].user = kstat_cpu(cpu).cpustat.user +
			...
			prev_cpu_data[cpu].io = kstat_cpu(cpu).cpustat.iowait;
		}
		return;
	}

	jiffies_difference = jiffies - prev_jiffies;
	...
	for_each_online_cpu(cpu) {
		...
		curr_cpu_data.user = kstat_cpu(cpu).cpustat.user +
		    kstat_cpu(cpu).cpustat.nice;

		curr_cpu_data.system = kstat_cpu(cpu).cpustat.system +
		    kstat_cpu(cpu).cpustat.softirq + kstat_cpu(cpu).cpustat.irq;

		curr_cpu_data.io = kstat_cpu(cpu).cpustat.iowait;
...
int kdbg_cpuusage_init(void)
{
	...
	cpuusage_auto_timer._softexpires = timespec_to_ktime(tv);
