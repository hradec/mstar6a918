...
/* write index of the circular buffer array */
...
static void topthread_show_given_entry(int idx, int disp_option)
{
	...
	{
		int total_cpu_per = 0;
		PRINT_KD("%5ld Sec ", g_ptopthread_proc_buff_[idx].sec);

		for (cpu = 0; cpu < NR_CPUS; cpu++)
			total_cpu += g_ptopthread_proc_buff_[idx].total[cpu];
		...
		while (loop < TOPTHREAD_MAX_ITEM) {
			...
			if (g_ptopthread_proc_buff_[idx].
					item_per_pid[loop].th_info.pid != 0) {

				PRINT_KD("%-9.9s %4d",
						convert_taskname(g_ptopthread_proc_buff_
						[idx].item_per_pid[loop].name, out),
						g_ptopthread_proc_buff_[idx].
						item_per_pid[loop].th_info.
						pid);

				PRINT_KD("(%3d%%) ",
						JIFFI_TO_PERCENT(
							g_ptopthread_proc_buff_
	...
	default:
	{
		...
		total_cpu = JIFFI_TO_PERCENT(g_ptopthread_proc_buff_[idx].total[cpu]);
		...
		while (loop < TOPTHREAD_MAX_ITEM) {
			if (g_ptopthread_proc_buff_[idx].item_per_cpu[cpu].cpu_table[loop].pid !=  0) {
				tick_cnt = JIFFI_TO_PERCENT(g_ptopthread_proc_buff_[idx].item_per_cpu[cpu].
									  cpu_table[loop].tick_cnt);
				if (tick_cnt > (TOPTHREAD_PERCENTAGE))
					tick_cnt = TOPTHREAD_PERCENTAGE;

				PRINT_KD("%-9.9s %4d",
						convert_taskname(g_ptopthread_proc_buff_[idx].
		...
		if (g_ptopthread_proc_buff_[idx].total[cpu] > topthread_cpu_time) {
			excess_usage =
				(g_ptopthread_proc_buff_[idx].total[cpu]
				 - topthread_cpu_time);
		} else {
			excess_usage = 0;
		}
...
static void topthread_show_given_entry(int idx, int disp_option)
{
	...
	PRINT_KD("%3d%% ",
		 JIFFI_TO_PERCENT(g_ptopthread_proc_buff_[idx].total[cpu]));

	/* to show pid & cpu usage */
	while (loop < TOPTHREAD_MAX_ITEM) {
		if (g_ptopthread_proc_buff_[idx].item_per_cpu[cpu].
		    cpu_table[loop].pid != 0) {
			PRINT_KD("%-9.9s %4d",
				 convert_taskname(g_ptopthread_proc_buff_[idx].item_per_cpu[cpu].
				 name[loop], out),
				 g_ptopthread_proc_buff_[idx].item_per_cpu[cpu].
				 cpu_table[loop].pid);
			PRINT_KD("(%3d%%) ",
				 JIFFI_TO_PERCENT(g_ptopthread_proc_buff_[idx].item_per_cpu[cpu].
				 cpu_table[loop].tick_cnt));
		} else {
			PRINT_KD("              (0%%) ");
		}
...
/*
 *  these 3 funciton are used for maintaing display buffer index
...
void topthread_find_top5thread_n_display(int buf_index)
{
	...
	int proc_buff_index = topthread_get_proc_index();
	...
	WARN_ON(!g_ptopthread_proc_buff_);
	...
	/* initialize to zero */
	memset(&g_ptopthread_proc_buff_[proc_buff_index], 0,
	 ...
	 */
	g_ptopthread_proc_buff_[proc_buff_index].sec =
	...
	sec_topthread_assign_all_pid(&g_ptopthread_proc_buff_[proc_buff_index],
			g_thread_info_table_chain[buf_index]);

#ifdef CONFIG_SMP
	sec_topthread_assign_top_pid(&g_ptopthread_proc_buff_[proc_buff_index],
			g_thread_info_table_chain[buf_index]);
#endif /* CONFIG_SMP */
	...
	while (loop < TOPTHREAD_MAX_ITEM) {
		 ...
		 */
		for (cpu_idx = 0; cpu_idx < NR_CPUS; cpu_idx++) {
			if (g_ptopthread_proc_buff_[proc_buff_index].
					item_per_cpu[cpu_idx].cpu_table[loop].pid) {

				rcu_read_lock();
				/* get the data from rearranged array of thread cpu usage */
				tsk =
					find_task_by_pid_ns(g_ptopthread_proc_buff_
				...
				if (tsk) {
					get_task_comm(g_ptopthread_proc_buff_
							[proc_buff_index].
							item_per_cpu[cpu_idx].
							name[loop], tsk);
				} else {
					strncpy(g_ptopthread_proc_buff_
							[proc_buff_index].item_per_cpu
							[cpu_idx].name[loop], "<th_dead>",
							SEC_MAX_NAME_LEN - 1);
				}

				g_ptopthread_proc_buff_[proc_buff_index].
		...
		if (g_ptopthread_proc_buff_[proc_buff_index].item_per_pid[loop].
				th_info.pid) {
...
{
	static unsigned long prev_jiffi;
	...
	/* Let CPU 0 do this task */
	if (cpu == 0) {

		/* Calculate 1sec difference inorder to show the top N (configurable) thread info
		 * periodically */
		...
		if (diff_jiffi >= TOPTHREAD_UPDATE_TICKS) {
			...
			if (g_thread_info_table_chain[g_writer]) {
				...
				SEC_TOPTHREAD_DEBUG
				    ("CPU [%d] Posting work Jiffies: curr %lu prev %lu\n",
				     cpu, cur_jiffi, prev_jiffi);

				if (sec_topthread_notify_reader(g_writer)) {
...
void sec_topthread_destroy_impl(void)
{
	...
	PRINT_KD("TOPTHREAD Destroyed Successfuly\n");
}
...
static int sec_topthread_control(void)
{
	...
	do {
		...
		PRINT_KD("1.  Config : CPU (default: All)\n");
		PRINT_KD("2.  Show Total CPU Usage for All Threads\n");
		PRINT_KD("3.  Show Total CPU Usage for Top 5 Threads\n");
		PRINT_KD
		    ("4.  Show Total CPU Usage for Top 5 Threads (120 sec)\n");
		PRINT_KD("-----------------------------------------\n");
 ...
 * the Menu for SMP proessor contains 2 SubMenu
 * 1- CPU 0 statistics
 * 2- CPU 1 statistics
 * 3- Consolidated
 * */
...
static void csf_show_entry(int index)
{
	...
	if (print_option == NR_CPUS) {	/* Print Usage of all CPU's */
		...
		for_each_online_cpu(cpu) {
			total_cpu_idx +=
				g_ptopthread_proc_buff_[index].item_per_cpu[cpu].
				max_thread;
			cpu_prcn +=
				JIFFI_TO_PERCENT(g_ptopthread_proc_buff_[index].
						total[cpu]);
			total_ctx +=
				g_ptopthread_proc_buff_[index].
		...
		BUFF_APPEND
			("\n|[%5ld sec] CPU USAGE:%3d%%(MAXIMUM %3d%%) # task:%3d. # ctx:%7lu.  |",
			 g_ptopthread_proc_buff_[index].sec, cpu_prcn, TOTAL_PERCENTAGE, total_cpu_idx, total_ctx);
		...
		for (j = 0; j < online_cpu; j += 2) {
			max_loop =
				get_max_index(g_ptopthread_proc_buff_[index].item_per_cpu, j, 2);
			...
			for (cpu = j; cpu < j+2; cpu++) {
				cpu_prcn = JIFFI_TO_PERCENT(g_ptopthread_proc_buff_[index].total[cpu]);
			...
			for (i = 0; i < max_loop; i++) {
				/*Take RCU read lock register can be changed */
				for (cpu = j; cpu < j+2; cpu++) {
					if (g_ptopthread_proc_buff_[index].
						...
						cpu_prcn = JIFFI_TO_PERCENT (g_ptopthread_proc_buff_
									 [index].item_per_cpu[cpu].
...
#endif /*CONFIG_SMP */
	{
		...
		PRINT_KD("\n|[ %5ld sec ] CPU[%1d] USAGE: %3d%%      |",
				g_ptopthread_proc_buff_[index].sec, cpu,
				cpu_prcn);
		...
		for (i = 0;
			...
			cpu_prcn = JIFFI_TO_PERCENT (g_ptopthread_proc_buff_[index].
...
int kdbg_topthread_init(void)
{
	...
	topthread_auto_timer._softexpires = timespec_to_ktime(tv);
...
#endif
	kdbg_register("COUNTER MONITOR: CPU Usage for each thread",
