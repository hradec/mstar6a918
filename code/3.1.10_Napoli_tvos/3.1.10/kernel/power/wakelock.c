...
/*0: no lock and expired lock; -1: has lock; >0: all are expired lock, return max timeout*/
...
static void expire_wake_locks(unsigned long data)
{
	...
	if (has_lock == 0){
#if defined(CONFIG_MP_MSTAR_STR_BASE_DEBUG)
        pr_info("queue suspend work on expire wake locks\n");
#endif
...
{
#ifndef CONFIG_MSTAR_STR_ORIGINAL_FLOW
    if(is_mstar_str())
        return 0;
    request_suspend_state(PM_SUSPEND_ON);
    wake_lock(&main_wake_lock);
	if (debug_mask & DEBUG_SUSPEND)
		pr_info("power_resume hold main wakelock\n");
#endif
	return 0;
}
#endif
	...
	struct wake_lock *lock, long timeout, int has_timeout)
{
	...
	if (type == WAKE_LOCK_SUSPEND) {
		...
		if (expire_in > 0) {
			...
			if (expire_in == 0){
#if defined(CONFIG_MP_MSTAR_STR_BASE_DEBUG)
                pr_info("queue suspend work on wake lock internal\n");
#endif
				queue_work(suspend_work_queue, &suspend_work);
		    }
		}
...
void wake_unlock(struct wake_lock *lock)
{
	...
	if (type == WAKE_LOCK_SUSPEND) {
		...
		if (has_lock > 0) {
			...
			if (has_lock == 0){
#if defined(CONFIG_MP_MSTAR_STR_BASE_DEBUG)
                pr_info("queue suspend work on wake unlock\n");
#endif
				queue_work(suspend_work_queue, &suspend_work);
		    }
		}
