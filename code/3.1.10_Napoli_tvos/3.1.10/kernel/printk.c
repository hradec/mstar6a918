...
extern unsigned int ramlog_kernel_printk;
extern unsigned int ramlog_kernel_enable;
#endif
#endif
...
static void emit_log_char(char c)
{
	...
	#if (CONFIG_MP_DEBUG_TOOL_RAMLOG == 1 )
        #ifdef CONFIG_MSTAR_RAMLOG
		if(ramlog_kernel_printk==1 || ramlog_kernel_enable == 0 )
		LOG_BUF(log_end) = c;
		#else
		LOG_BUF(log_end) = c;
		#endif
...
extern int bRamlogInit ;

static char u8ramlogBuf[200];
static int  u32SizePrintk=0;
#endif
#endif
...
asmlinkage int vprintk(const char *fmt, va_list args)
{
	...
	if(bRamlogInit == 1)
	{
	    u32SizePrintk = vscnprintf(u8ramlogBuf,200,fmt, args);
		ram_log_write(1, u8ramlogBuf, u32SizePrintk);
		//ram_log_write(int ModuleID,const char* buff,int len)
	}
    #endif
    #endif
