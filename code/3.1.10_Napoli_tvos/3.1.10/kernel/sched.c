...
{
        #if defined(CONFIG_MP_SCHD_DEBUG_RE_SCHED_NOR) 
        struct sched_param lparam;
        int reset_on_fork = policy & SCHED_RESET_ON_FORK;
              
        memcpy(&lparam, param, sizeof(struct sched_param));

        if (((policy&(~SCHED_RESET_ON_FORK)) == SCHED_FIFO) || ((policy&(~SCHED_RESET_ON_FORK)) == SCHED_RR)) {
            printk("Warnning! We do not allow set thread to FIFO/RR real time scheduler!! Redirect it to normal\n");
            policy = SCHED_NORMAL|reset_on_fork;
            lparam.sched_priority = 0;
        }

	return __sched_setscheduler(p, policy, &lparam, true);
        #else
			       ...
			       const struct sched_param *param)
{
	...
	return __sched_setscheduler(p, policy, &lparam, false);
        #else
