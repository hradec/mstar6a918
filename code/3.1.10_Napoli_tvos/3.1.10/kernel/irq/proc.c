...
#include "chip_int.h"
#endif  /* End of CONFIG_MP_PLATFORM_ARCH_GENERAL */
...
ssize_t irq_proc_write(struct file *fp, const char *bufp, size_t len, loff_t *where)
{
    ...
    switch(enable)
    {
        case E_IRQ_ENABLE:
            ...
            // if irq has been enabled, does not need enable again
...
//	if (!(irq_desc[ip->irq].status_use_accessors & IRQD_IRQ_DISABLED))
	if((irq_to_desc(ip->irq)->status_use_accessors & IRQD_IRQ_DISABLED))
		disable_irq_nosync(ip->irq);
	return 0;
}
...
#ifdef CONFIG_MP_DEBUG_TOOL_INTERRUPT_DEBUG
extern unsigned int threshold;

static int interrupt_show(struct seq_file *m, void *v){
    seq_printf(m, "interrupt threshold %d\n",threshold);
    return 0;
}

static int interrupt_open(struct inode *inode, struct file *file)
{
    return single_open(file, interrupt_show, PDE(inode)->data);
}

static ssize_t interrupt_write(struct file *file,
const char __user *buffer, size_t count, loff_t *ppos)
{
    char procfs_buffer[40]={0};
    int time = 0;
    int err;
    if ((err = copy_from_user(procfs_buffer, buffer, count)))
        return err;
    sscanf(procfs_buffer,"%d",&time);
    printk("set interrupt %d\n",time);
    threshold = time;
    return count;
}

static const struct file_operations interrupt_proc_fops = {
    .open       = interrupt_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release    = single_release,
...
#ifdef CONFIG_MP_DEBUG_TOOL_INTERRUPT_DEBUG
    proc_create("interrupt_debug", 0600, NULL,&interrupt_proc_fops);
#endif
}
