...
struct mtd_info {
	...
	#if defined(CONFIG_MTD_UBI_WRITE_CALLBACK) && (MP_NAND_UBI == 1)
	int (*write_cb)(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf,
					struct write_info *write);
	#endif

	#if defined(CONFIG_MTD_SLC_WRITE) && (MP_NAND_MTD == 1)
	int (*read_slc) (struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf);


	int (*write_slc) (struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf);
	#endif
