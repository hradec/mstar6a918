...
static void sd_shutdown(struct device *dev)
{
	...
	if (!sdkp)
		...
		if (sdkp->WCE) {
		 ...
		 /*
			if  (MP_SCSI_HD_SUSPEND == 1) we need to mark this because the QA disk
			has no cache but the head is declared support
		*/
	#endif
...
static int sd_suspend(struct device *dev, pm_message_t mesg)
{
	...
	//if (MP_SCSI_HD_SUSPEND == 1), we need to mark this because the QA disk has no cache but the head is declared  support
	#endif
