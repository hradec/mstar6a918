...
static int n_tty_open(struct tty_struct *tty)
{
	...
	if (!tty->read_buf) {
		tty->read_buf = vzalloc(N_TTY_BUF_SIZE);
		if (!tty->read_buf)
			return -ENOMEM;
	}
	if (!tty->echo_buf) {
		tty->echo_buf = vzalloc(N_TTY_BUF_SIZE);
...
#if defined(CONFIG_MP_PLATFORM_ARCH_GENERAL)
#ifdef KDEBUGD_STRING
		sprintf(sched_serial, "%s", KDEBUGD_STRING);
		printk(KERN_EMERG "[KDEBUGD] enter : %s\n\n", KDEBUGD_STRING);
#else
        printk("\033[35mFunction = %s, Line = %d, [KDEBUGD] enter : NA\033[m\n", __PRETTY_FUNCTION__, __LINE__);
#endif
#endif
