...
static int nand_ReadDisturbance_BigImg(struct mtd_info *mtd, uint32_t u32_from)
{
	...
	{
		u32_Err = mtd->block_isbad(mtd,u16_GoodBlock_Idx<<u32_phys_erase_shift);
		...
		if(u32_Err == 0)
		{
            ...
            mtd->block_markbad(mtd,u16_GoodBlock_Idx<<u32_phys_erase_shift);
	}
    ...
    for(u16_i = 0; u16_i < u16_BlkPageCnt; u16_i ++)
    {
		...
		u32_Err = mtd->read_oob(mtd, u32_Row, &ops);
        ...
        u32_Err = mtd->write_oob(mtd, u32_Row, &ops);
		if (u32_Err!= 0)
		{
		    kfree(pu8_PageDataBuf);
	        kfree(pu8_PageSpareBuf);
		...
		else
		{
			if(pu8_PageSpareBuf[1] == 0x36)
			{
				...
				if(u32_Err != 0)
				{
					...
					mtd->block_markbad(mtd,u16_BlkIdx<<u32_phys_erase_shift);
				    kfree(pu8_PageDataBuf);
	                kfree(pu8_PageSpareBuf);	
					return u32_Err;
				}
...
static int mtdblock_open(struct mtd_blktrans_dev *mbd)
{
	...
	if(mtd->type == MTD_NANDFLASH)
	{
		...
		{
		    ret = mtd->block_isbad(mtd,u16_blkidx<<u32_phys_erase_shift);
			if(ret != 0)//bad block
			{
