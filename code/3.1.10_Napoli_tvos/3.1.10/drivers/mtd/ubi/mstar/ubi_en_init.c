...
__u32 bfind_first_empty_page(struct ubi_device * ubi, __u32 blk_idx, int blk_page_count)
{
	...
	u32_err = ctrl_mtd->read(ctrl_mtd, blk_idx << ctrl_mtd->writesize_shift, 
	...
	u32_err = ctrl_mtd->read(ctrl_mtd, (blk_idx + blk_page_count - 1) << ctrl_mtd->writesize_shift, 
		ctrl_mtd->writesize, &read, buf);
	if(u32_err)
	{
		if (u32_err!= -EUCLEAN && u32_err!= (-EUCLEAN - 1) && u32_err != (-EUCLEAN - 2))
	...
	while( p1 < p2 -1)
	{
		pn = (p1+p2)/2;
		//read page pn
		u32_err = ctrl_mtd->read(ctrl_mtd, pn << ctrl_mtd->writesize_shift, 
		ctrl_mtd->writesize, &read, buf);
		if(u32_err)
		{
			if (u32_err!= -EUCLEAN && u32_err!= (-EUCLEAN - 1) && u32_err != (-EUCLEAN - 2))
...
int find_first_empty_page(struct ubi_device * ubi, int blk_idx, int blk_page_count)
{
	...
	while(--pn >= blk_idx)
	{
		...
		err = ctrl_mtd->read(ctrl_mtd, pn << ctrl_mtd->writesize_shift, 
		ctrl_mtd->writesize, &read, buf);
		if(err)
		{
...
int erase_all_blocks(void)
{
	...
	for(u32_i = 0; u32_i < cb_count; u32_i += blk_page_count)
	{
		err = tbl_mtd->block_isbad(tbl_mtd, u32_i << tbl_mtd->writesize_shift);
		if(err == 1)
		{
			eninit_msg("Skip bad block %X", u32_i);
			continue;
...
int process_ctrl_record(struct ubi_device *ubi, __u8 * pu8_DataBuf)
{
	...
	{
		err = ctrl_mtd->block_isbad(ctrl_mtd, i << ctrl_mtd->writesize_shift);
		if(err > 0)
		{
		...
		if( first_empty == i)
			continue;
		//read first_empty -1;
		err = ctrl_mtd->read(ctrl_mtd, (first_empty - 1) << ctrl_mtd->writesize_shift, 
	...
	if(bitmap_block != -1)
	{
		...
		err = ctrl_mtd->read(ctrl_mtd, (bitmap_page) << ctrl_mtd->writesize_shift, 
			ctrl_mtd->writesize, &read, pu8_DataBuf);
	...
	for(i = 0; i < tbl_mtd->writesize - sizeof(struct ubi_en_bitmap_hdr); i ++)
	{
		...
		for(j = 7; j >= 0 ; j --)
		{
			...
			if(v)
			{
				...
				err = tbl_mtd->read(tbl_mtd, page_idx << tbl_mtd->writesize_shift, 
	...
	for(j = 0; j < tsm->groups_num; j ++)
	{
		if(!tsm->valid_groups[j] || all_sync)
		{
			...
			for(i = j * n ; i < end_idx ; i ++)
			{
				...
				{
					err = tbl_mtd->write(tbl_mtd, ubi->ebr->next_avail_tbl_pages <<  tbl_mtd->writesize_shift, 
...
retry:	
	...
	err = ctrl_mtd->write(ctrl_mtd, page_idx << ctrl_mtd->writesize_shift, ctrl_mtd->writesize, &written, pu8_DataBuf);
	if(err)
	{
		eninit_msg("write fail at %X, mark it bad",page_idx << ctrl_mtd->writesize_shift);
		ctrl_mtd->block_markbad(ctrl_mtd, (page_idx/blk_page_count) << ctrl_mtd->erasesize_shift);
		ubi->ebr->avail_ctrl_blk_num --;
