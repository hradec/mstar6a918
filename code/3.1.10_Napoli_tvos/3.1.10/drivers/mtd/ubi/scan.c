...
#include "../../mstar/nand/fcie3/inc/api/drv_unfd.h"
...
int process_tbl_page(struct ubi_device *ubi, struct ubi_scan_info *si, 	__u8 *pu8_DataBuf, __u32 page_idx)
{
	...
	for(idx = 0; idx < ttl_rcd; idx ++)
	{
		...
		if (ubi->image_seq && tmp_ebr.image_seq &&
			...
			err = add_to_list(si, pnum, UBI_SCAN_UNKNOWN_EC, 0, &si->erase);
			if (err)
				return err;
			continue;
		}
	...
	if (!sv) {
		...
		if(corrupt) {
			...
			retry:
				...
				err = add_to_list(si, old_seb->pnum, old_seb->ec, 0, &si->erase);
				if (err){
					ubi_free_vid_hdr(ubi, vid_hdr);					
					goto out_free;
				}
...
write_error:
	 ...
	 */
	err = add_to_list(si, new_seb->pnum, new_seb->pnum, 0,
						  &si->corr);
