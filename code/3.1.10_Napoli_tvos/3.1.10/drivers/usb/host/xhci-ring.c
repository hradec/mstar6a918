...
extern int handshake(struct xhci_hcd *xhci, void __iomem *ptr,
		...
		struct xhci_event_cmd *event)
{
	...
	if ((GET_COMP_CODE(le32_to_cpu(event->status)) == COMP_CMD_ABORT) ||
		...
		{
			//If dequeue is equal to enqueue, return here.
			//Don't let following code to increase dequeue.
...
#if MP_USB_MSTAR
	hcd = xhci_to_hcd(xhci);  //patch for compile warning.
#endif 
