...
#if defined(ENABLE_12US_EOF1) && (MP_USB_MSTAR==1)
...
static void tdi_reset (struct ehci_hcd *ehci)
{
	 ...
	 */
#if defined(CONFIG_MIPS) && (MP_USB_MSTAR==1)
...
/*-------------------------------------------------------------------------*/
#if (_USB_T3_WBTIMEOUT_PATCH) && (MP_USB_MSTAR==1)
...
static int ehci_init(struct usb_hcd *hcd)
{
	...
	if (HCC_CANPARK(hcc_params)) {
		 ...
		 */
#if (MP_USB_MSTAR==1) && defined(ENABLE_12US_EOF1) && defined(ENABLE_CHIPTOP_PERFORMANCE_SETTING)
		int chipVER = readw((void *)(MSTAR_CHIP_TOP_BASE+0xCE*2));
		park = (chipVER != 0x100) ? 3 : 0;
#endif
		if (park) {
