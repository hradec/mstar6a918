...
#if (MP_NTFS_REFINE_READDIR==1)
typedef struct
{   
   int          pre_uname_len;
   ntfschar  pre_uname[NTFS_MAX_NAME_LEN];
}ntfs_dir_private_info;
#endif
...
#if (MP_NTFS_REFINE_READDIR==1)
static int ntfs_idx_cmp_uname(const ntfschar *name1, const u32 name1_len,
		const ntfschar *name2, const u32 name2_len,
		const ntfschar *upcase, const u32 upcase_len)
{
	int rc;
	rc = ntfs_collate_names(name1, name1_len,
				                  name2, name2_len, 1,
		                     IGNORE_CASE, upcase, upcase_len);
	if(rc)
		return rc;
	return ntfs_collate_names(name1, name1_len,
				                  name2, name2_len, 1,
			                     CASE_SENSITIVE, upcase, upcase_len);
}
static int ntfs_browser_dir_ia(ntfs_inode *dir_ni, void *dirent,  ntfs_inode *ia_ni, ntfschar *pre_uname,
		 int *pre_uname_len, int skip_pre, VCN vcn, u8 index_vcn_size_bits, u32 index_block_size, s64 *pos, filldir_t filldir)
{
	INDEX_ALLOCATION *ia;
	u8 *index_end;
	INDEX_ENTRY *ie;
	...
	if (sle64_to_cpu(ia->index_block_vcn) != vcn)
	{
		ntfs_error(sb, "Actual VCN (0x%llx) of index buffer is different "
	...
	{
		ntfs_error(sb, "Index buffer (VCN 0x%llx) of directory inode 0x%llx "
				"has a size (%u) differing from the directory "
				"specified size (%u).\n", (long long)vcn,
				(unsigned long long)ia_ni->mft_no,
	...
	{
		ntfs_error(sb, "Size of index buffer (VCN 0x%llx) of directory inode "
				"0x%llx exceeds maximum size.\n",
				(long long)vcn, (unsigned long long)ia_ni->mft_no);
		goto err_out;
	}
	/* The first index entry. */
	ie = (INDEX_ENTRY*)((u8*)&ia->index +
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length)))
	{
		...
		{
			ntfs_error(sb, "Index entry out of bounds in directory "
				       "inode %lld.\n", 
				       (unsigned long long)ia_ni->mft_no);
			goto err_out;
		}
		...
		{
			ntfs_error(sb, "Zero length index entry in inode %lld"
				       "\n", (unsigned long long)ia_ni->mft_no);
			goto err_out;
		}
		/*
		 ...
		 * know which way in the B+tree we have to go.
		 */
		if(skip_pre && !(ie->flags & INDEX_ENTRY_END))
		{
			rc = ntfs_idx_cmp_uname(pre_uname, *pre_uname_len,
				        (ntfschar*)&ie->key.file_name.file_name,
				        ie->key.file_name.file_name_length, dir_ni->vol->upcase, dir_ni->vol->upcase_len);
			if (rc > 0)
			    continue;
			if (rc<0)
			{	
				if(ie->flags & INDEX_ENTRY_NODE)
				{
					...
					res = ntfs_browser_dir_ia(dir_ni, dirent,  ia_ni, pre_uname,
		                                     pre_uname_len, skip_pre, vcn, index_vcn_size_bits, index_block_size, pos, filldir);
					if(res < 0)
						goto err_out;
					else if(res > 0)
						goto done;
				...
				ia_mapping = VFS_I(dir_ni)->i_mapping;
				ia_page = ntfs_map_page(ia_mapping, vcn <<
				dir_ni->itype.index.vcn_size_bits >> PAGE_CACHE_SHIFT);
				if (IS_ERR(ia_page))
				{
					ntfs_error(sb, "Failed to map directory index page, error %ld.", -PTR_ERR(ia_page));
					ia_page = NULL;
					goto err_out;
				}
				lock_page(ia_page);
				rc = ntfs_filldir(vol, pos, dir_ni, ia_page, ie, name, dirent, filldir);
				unlock_page(ia_page);
				...
				memcpy(pre_uname, &ie->key.file_name.file_name, sizeof(ntfschar)*ie->key.file_name.file_name_length);
                           *pre_uname_len = ie->key.file_name.file_name_length;
			}
			skip_pre = 0;
			continue;
		}
...
static int ntfs_browser_dir_ir(ntfs_inode *dir_ni,  void *dirent, ntfschar *pre_uname,
		int *pre_uname_len, int skip_pre, s64 *pos, filldir_t filldir)
{
	VCN vcn;
	ntfs_volume *vol = dir_ni->vol;
	struct super_block *sb = vol->sb;
	...
	if (ntfs_attr_lookup(AT_INDEX_ROOT, I30, 4, CASE_SENSITIVE, 0, NULL,
			0, ctx)) 
	{
		ntfs_error(sb,"Index root attribute missing in directory inode "
	...
	{
		ntfs_error(sb, "Index block size %u is invalid.\n",
				(unsigned)index_block_size);
		goto err_out;
	}
	index_end = (u8*)&ir->index + le32_to_cpu(ir->index.index_length);
	...
	if (IS_ERR(ia_vi)) 
	{
  		ntfs_error(sb,"Failed to open index allocation (inode %lld)",
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) 
	{
		...
		if ((u8*)ie < (u8*)ctx->mrec || (u8*)ie +
				...
				index_end) 
		{
			ntfs_error(sb, "Index entry out of bounds in inode %lld"
				       "\n", (unsigned long long)dir_ni->mft_no);
			goto err_out;
		}
		if (!le16_to_cpu(ie->length)) 
		...
		lock_page(ia_page);
		rc = ntfs_filldir(vol, pos, dir_ni, ia_page, ie, name, dirent,
				filldir);
    	if (ia_page)
		{
			unlock_page(ia_page);
...
static int ntfs_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	int rc, err=0;
	int res=0;
	ntfs_dir_private_info *info = NULL;

	fpos = filp->f_pos;
	ntfs_debug("Entering for inode 0x%lx, fpos 0x%llx.", vdir->i_ino, fpos);
	
	info = (ntfs_dir_private_info *) (filp->private_data);  
	if(!info)
	{
		info = ntfs_malloc_nofs((unsigned long)sizeof(ntfs_dir_private_info));
		if(!info)
		{
			err = ENOMEM;
			goto err_out;
	...
	res = ntfs_browser_dir_ir( ndir,  dirent, info->pre_uname,
		               &info->pre_uname_len, info->pre_uname_len, &fpos, filldir);
	if(res < 0)
		goto err_out;
...
static int ntfs_ioctl_volume_id(struct inode *dir, unsigned long arg)
{
    ...
    ntfs_volume *vol = NTFS_SB(sb);
    char vn[64]={0};
	/*N.B. To print a 64-bit variable,should write "%llu"(should not write "%ull").
    ...
    copy_to_user(v,vn, NTFS_VOLUME_NAME_LEN );
...
#if (MP_NTFS_REFINE_READDIR==1)
static int ntfs_release_dir (struct inode * inode, struct file * filp)
{
       if (filp->private_data)
		ntfs_free(filp->private_data);
	return 0;
}
#endif
