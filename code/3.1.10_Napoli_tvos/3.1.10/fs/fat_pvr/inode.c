...
static int fat_fill_inode(struct inode *inode, struct msdos_dir_entry *de)
{
	...
	inode->i_generation = get_seconds();
        #if (MP_SYSATTR_SHOW == 1)
		if(de->attr & ATTR_HIDDEN)
		{	
			MSDOS_I(inode)->i_attrs |=ATTR_HIDDEN;
				//printk("set hid 1 \n");
		}
		else
		{
...
static void fat_write_super(struct super_block *sb)
{
	...
	struct address_space *mapping = sbi->fattab_inode->i_mapping;
	int ret;
	sb->s_dirt = 0;
	...
	*/
	//WARN_ON(1);
	ret = sync_inode(sbi->fattab_inode, &wbc);
	if (!ret)
	       ret = filemap_fdatawrite(mapping);
...
retry:
	...
	if (!bh) {
		fat_msg(sb, KERN_DEBUG, "unable to read inode block "
		   ...
		   void (*setup)(struct super_block *))
{
	...
	if (!sbi->fat_length && b->fat32_length) {
		...
		if (fsinfo_bh == NULL) {
			fat_msg(sb, KERN_DEBUG, "bread failed, FSINFO block"
