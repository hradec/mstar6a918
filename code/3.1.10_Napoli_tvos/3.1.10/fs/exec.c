...
#if defined(CONFIG_MSTAR_COREDUMP_REBOOT) && (CONFIG_MSTAR_COREDUMP_REBOOT == 1)
...
#if defined(CONFIG_MP_DEBUG_TOOL_COREDUMP_DETECT_USB_PLUGIN)
...
#endif /* CONFIG_MP_DEBUG_TOOL_COREDUMP_DETECT_USB_PLUGIN */
...
#if (MP_DEBUG_TOOL_COREDUMP_PATH_OPTION == 1)
	#ifdef CONFIG_MSTAR_COREDUMP_PATH_BOOTARGS
...
bool query_mount_state(char *con_file)
{
    ...
    mount_result=0;
		...
		for (i=0;i<=1000;i++)
{
		if(buf[i]=='/' && buf[i+1]=='u' && buf[i+2]=='s' && buf[i+3]=='b' && buf[i+4]=='/'&& buf[i+5]=='s'&& buf[i+6]=='d'&& buf[i+7]=='a'&& buf[i+8]=='1')
		{
		  mount_result=1;
		}
}
...
bool query_partitions_state(char *con_file)
{
    ...
    partitions_result=0;
		for (i=40;i<=800;i++)
{
		if(buf[i]==32 && buf[i+1]=='s' && buf[i+2]=='d' && buf[i+3]=='a' && buf[i+4]==10)
		{
		    partitions_result=1;
		}
		if(buf[i]==32 && buf[i+1]=='s' && buf[i+2]=='d' && buf[i+3]=='a' && buf[i+4]=='1'&& buf[i+5]==10)
		{
		    partitions_result=1;
		}
		if(buf[i]==32 && buf[i+1]=='s' && buf[i+2]=='d' && buf[i+3]=='b' && buf[i+4]==10)
		{
		    partitions_result=1;
		}
		if(buf[i]==32 && buf[i+1]=='s' && buf[i+2]=='d' && buf[i+3]=='b' && buf[i+4]=='1'&& buf[i+5]==10)
		{
		    partitions_result=1;
		}
...
#endif
#endif
#if defined(CONFIG_MP_DEBUG_TOOL_CODEDUMP) && !defined(CONFIG_MP_DEBUG_TOOL_COREDUMP_PATH)
...
#if defined(CONFIG_MP_DEBUG_TOOL_COREDUMP_DETECT_USB_PLUGIN)
        if (mod_usbcore && mod_ehci && mod_storage && partitions_result==1 && mount_result==1)
#else
...
detect:
       ...
       else
       {
        	if (mount_path_num == 0xFF)
              ...
              #if defined(CONFIG_MP_DEBUG_TOOL_FORCE_IGNORED_CORE_DUMP_PATH_BOOTARGS_WHEN_USB_PLUGIN)
              {
                    cprm.file = filp_open(cn.corename, O_CREAT | 2 | O_NOFOLLOW | O_LARGEFILE | flag, 0600);
                   if (!IS_ERR(cprm.file))
                           {
             ...
             #endif /* CONFIG_MP_DEBUG_TOOL_FORCE_IGNORED_CORE_DUMP_PATH_BOOTARGS_WHEN_USB_PLUGIN */
      }
#endif
...
#if defined(CONFIG_MP_DEBUG_TOOL_CODEDUMP_DATA_SYNC)
       sys_sync();
#endif /* CONFIG_MP_DEBUG_TOOL_CODEDUMP_DATA_SYNC */
